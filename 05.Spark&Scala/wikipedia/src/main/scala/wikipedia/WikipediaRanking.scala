...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)(
      { (acc, value) => if (value.mentionsLanguage(lang)) acc + 1 else acc },
      (acc1, acc2) => acc1 + acc2)
  ...
  //    index
  //      .collect()
  //      .map(index => (index._1, index._2.size))
  //      .toList
  //      .sortBy({ case (_, article) => article })
  //    .reverse
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd
      .flatMap((x: WikipediaArticle) =>
        langs.map((l: String) => (l, if (x.mentionsLanguage(l)) 1 else 0)))
      .reduceByKey(_ + _)
