...
네스테로프 모멘텀 최적화 또는 가속 경사(NAG) 라고 모멘텀보다 항상 거의 다 빠름.
...
적응적 모멘트 추정(Adaptive moment estimation)을 의미하는 Adam은 모멘텀 최적화와 RMSProp의 아이더를 합친 것.

모멘텀 최적화처럼 지난 그래디언트의 지수 감소 평균을 따르고 RMSProp철 밎난 그래디언트 제곱의 지수 감소된 평균을 따른다.
